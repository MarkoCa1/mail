...
class Smtp extends Config
{
	...
	{
		// 握手
		$this->execute("EHLO " . str_replace('ssl://', '', $this->smtp));
		// 登录验证
...
//			exit();
			...
			{
				if ($command == 'QUIT' && isset($enable_crypto_result))
				{
					stream_socket_enable_crypto($this->stream, false, STREAM_CRYPTO_METHOD_TLS_CLIENT);
				}
				...
				if ($command == 'STARTTLS')
				{
					$enable_crypto_result = stream_socket_enable_crypto($this->stream, true, STREAM_CRYPTO_METHOD_TLS_CLIENT);
